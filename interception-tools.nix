{ config, pkgs, ... }:
{
  ...
  services.interception-tools =
    ...
    in
    {
      ...
      udevmonConfig = ''
        # - CMD: mux -c mouse-modifiers
        # - JOB: mux -i mouse-modifiers | dual-function-keys -c ${dualFunctionKeysConfig} | uinput -c ${hybridConfig}
        # - JOB: intercept $DEVNODE | mux -o mouse-modifiers
        #   DEVICE:
        #     LINK: /dev/input/by-id/usb-Lenovo_ThinkPad_Compact_USB_Keyboard_with_TrackPoint-event-kbd
        # - JOB: intercept -g $DEVNODE | mux -o mouse-modifiers
        #   DEVICE:
